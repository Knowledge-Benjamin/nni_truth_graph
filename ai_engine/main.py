...
def extract_claims(request: TextRequest):
    print(f"Processing article: {request.article_id} (Mode: {EXECUTION_MODE})")
    timestamp = datetime.datetime.now().isoformat()
    
    try:
        extracted_text = request.content
        
        # Only run Summarization if in LOCAL mode and pipeline is loaded
        if EXECUTION_MODE == "local" and extractor_pipeline and len(request.content) > 1000:
            print("DEBUG: Running Local Summarization...")
            try:
        ...
        for i, s in enumerate(sentences):
            if len(s) > 15:
                # 1. Gather Evidence (External APIs always run)
                try:
                    fact_check = check_google_fact_check(s)
                    citations = find_citations_with_date(s)
                except Exception:
                    citations = []
                    fact_check = None
