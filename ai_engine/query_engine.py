...
class GraphSearcher:
    """
    Advanced Search for Truth Graph using Neo4j.
    Supports: Hybrid Search (Vector + Keyword) and Evolution Traversal.
    """
    def __init__(self):
        ...
        try:
            ...
            print("✅ GraphSearcher connected to Neo4j")
        except Exception as e:
            print(f"⚠️  GraphSearcher Init Failed: {e}")
            self.driver = None

    def hybrid_search(self, query_text, embedding=None, limit=10):
        """
        Combine Vector Search + Keyword Search.
        Prerequisite: Embedding must be generated by SemanticLinker before calling.
        """
        ...
        with self.driver.session() as session:
            ...
            if embedding:
                vector_res = session.run("""
                    ...
                    YIELD node, score
                    RETURN node.id as id, node.statement as text, score, 'vector' as method
            ...
            # Sanitise query for Lucene
            safe_query = query_text.replace("'", "").replace('"', '')
            keyword_res = session.run("""
                CALL db.index.fulltext.queryNodes("fact_statement_fulltext", $query) 
                YIELD node, score
                RETURN node.id as id, node.statement as text, score, 'keyword' as method
            """, query=f"{safe_query}", limit=limit)
            
            for record in keyword_res:
                rid = record['id']
                if rid in results:
                    # Boost score if found in both
                    results[rid]['score'] += (record['score'] * 0.5) 
                    results[rid]['method'] = 'hybrid'
                else:
                    results[rid] = {
                        "id": rid,
                        "text": record['text'],
    ...
    def get_fact_history(self, fact_id):
        """
        Traverse :SUPERSEDES relationships to show evolution.
        Direction: (Older) <-[:SUPERSEDES]- (Newer)
        Find both ancestors (past versions) and descendants (updates).
        """
        if not self.driver: return None
        
        with self.driver.session() as session:
            # 1. Find Ancestors (What did this supersede?)
            ancestors = session.run("""
                MATCH path = (f:Fact {id: $id})-[:SUPERSEDES*]->(old:Fact)
                RETURN [n in nodes(path) | {id: n.id, text: n.text, date: n.created_at}] as history
            """, id=fact_id)
            
            # 2. Find Descendants (Has this been superseded?)
            descendants = session.run("""
                MATCH path = (new:Fact)-[:SUPERSEDES*]->(f:Fact {id: $id})
                RETURN [n in nodes(path) | {id: n.id, text: n.text, date: n.created_at}] as future
            """, id=fact_id)
            ...
            for r in ancestors:
                history_chain = r['history'] # Path returns list
                
            for r in descendants:
                future_chain = r['future']
                
            return {
                "past": history_chain[1:] if history_chain else [], # Skip self [0]
                "future": future_chain[:-1] if future_chain else [] # Skip self [last]
            }
