...
**HIGH (Fix Second):** 5. Don't use logging in signal handlers, use direct sys.write() only 6. Wrap trafilatura.fetch_url() with timeout 7. Add connection timeout and statement timeout to psycopg2 8. Make database queries non-blocking with asyncpg if possible

**MEDIUM (Fix Third):** 9. Set `PYTHONUNBUFFERED=1` in Dockerfile (already done ✓) 10. Add explicit timeout handling for each async task 11. Implement proper async database access instead of sync

**LOW (Nice to Have):** 12. Implement asyncio task timeouts at event loop level 13. Use structured logging with proper error handling 14. Add health checks to orchestrator
